(function(W, D, $) {
	...
	$main.ui = {
		...
		changeCharts: function(target, value) {
			...
			if(target == 'One'){
				...
				for(var i=1; i<4; i++){
					$("#recallChart7-"+i+"Div").hide();
					$("#recallChart8-"+i+"Div").hide();
				}
				$("#recallChart7-"+value+"Div").show();
				$("#recallChart8-"+value+"Div").show();
			}
			else if(target == 'Five'){
		...
		makeCharts: function() {
			...
			// 국내
			var ctx1 = document.getElementById('recallChart1').getContext('2d');
		...
		/**
		 * @name         : makeCharts4
		 * @description  : 월별,연도별 접수경로별 현황 차트 생성	
		 * @date         : 2019. 12. 07.  
		 * @author	     : 박소희
		 */
		makeCharts4: function() {

			var params = fn_getFormData($("#srchFrm"));
            params.statType = 'SC'; //신고
            params.chartType = 'RP'; //접수경로별
            
			ajax(true, contextPath + '/si/statsInfo/chartList.do', 'body', '처리중입니다.', params, function(dataAjax) {
				if(dataAjax != null) {

			        //국내
					var labels4_1 = new Array();
					var cData4_1 = new Array();

			        $.each(dataAjax.korList, function(index, data){
			        	labels4_1.push(data.recpName);
			        	cData4_1.push(data.recallCarCount);
			        });
			        
			        makeDoughnutCharts(labels4_1, cData4_1, 'recallChart7');
			        makeBarCharts(labels4_1, cData4_1, 'recallChart7-2', '국산');
					makeLineCharts(labels4_1, cData4_1, 'recallChart7-3', '국산');
			        
		            //수입
					var labels4_2 = new Array();
					var cData4_2 = new Array();
					
			        $.each(dataAjax.forList, function(index, data){
			        	labels4_2.push(data.recpName);
			        	cData4_2.push(data.recallCarCount);
			        });
			        
			        makeDoughnutCharts(labels4_2, cData4_2, 'recallChart8');
			        makeBarCharts(labels4_2, cData4_2, 'recallChart8-2', '수입');
					makeLineCharts(labels4_2, cData4_2, 'recallChart8-3', '수입');
		...
		makeCharts5: function() {
			...
			ajax(true, contextPath + '/si/statsInfo/chartList.do', 'body', '처리중입니다.', params, function(dataAjax) {
				if(dataAjax != null) {
			        ...
			        $.each(dataAjax.forList, function(index, data){
			        	labels5_2.push(data.groupCarName);
	...
	$main.event = {
			...
			btnListClickHandler : function() {
		    	...
		    	postToURL(contextPath+'/si/statsInfo/scList.do', data);
 ...
 * @author	     : 박소희
 */
function makeDoughnutCharts(labels, cData, PrdctId){
	...
	var ctx = document.getElementById(PrdctId).getContext('2d');
