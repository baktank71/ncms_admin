		...
		<div class="content-padder">
            <div class="uk-section-small">
                <div class="uk-container uk-container-large">
                    <h3>게시물 관리</h3><!-- 리콜현황(자동차) -->
                    <a href="#" class="uk-button uk-button-default board-select" id="selectBoxName"> 리콜현황 <span uk-icon="chevron-down"></span></a>
                            ...
                            <div class="uk-card uk-card-default uk-card-body">
                                    ...
                                    <div class="uk-form-controls">
		                            	<div class="uk-form-controls">
		                            		<select id="ctype" name="ctype" class="uk-select w-100">
		                            			<option value="" <c:if test="${params.ctype ne 'C'}">selected </c:if>>자동차</option>
		                            			<option value="C" <c:if test="${params.ctype eq 'C'}">selected </c:if>>건설기계</option>
		                            		</select>
		                            	</div>
                                ...
                                <table class="uk-table uk-table-divider board-list">
                                    ...
                                    <tbody class="list">
                                     	<c:choose> 
		                                    ...
		                                    <tr>
	                                        	<td><input class="uk-checkbox" type="checkbox" id="ckId" value="${data.recallId},${data.ctype}"></td>
	                                        	<td class="num"><c:out value="${num}"/></td>
	                                            <td class="subject uk-text-left"><a href="#" onclick="$rcInfo.event.detailView('${data.recallId}','1001','${data.ctype}'); return false;"><c:out value="${data.recallTitle}"/></a></td>
	                                            <td class="writer"><c:out value="${data.recallSource}"/></td>
                                     	...
                                     	<%-- <c:choose> 
                                     		<c:when test = "${empty result || result[0].totalCount == 0 }"> 
                                     			<tr> 
                                     				<td colspan="15">검색 내역이 없습니다.</td> 
                                     			</tr> 
                                     		</c:when> 
                                     		<c:otherwise>
                                     		<c:set var="num" value="${paginationInfo.totalRecordCount - ((paginationInfo.currentPageNo -1) * 10) }" />
                                        	<c:forEach items="${result}" var="data" varStatus="status">
		                                    <tr onclick="$rcInfo.event.detailView('<c:out value="${data.recallId}"/>', '1001'); return false;" style="cursor: pointer;">
	                                        	<td><input class="uk-checkbox" type="checkbox" id="ckId" value="${data.recallId}"></td>
	                                        	<td class="date"><c:out value="${data.recallYy}"/></td>
	                                            <td class="writer"><c:out value="${data.originalMakeNm}"/></td>
	                                            <td class="subject"><c:out value="${data.productName}"/></td>
	                                            <td class="date"><c:out value="${data.productDateFrom}"/> ~ <c:out value="${data.productDateTo}"/></td>
	                                            <td class="date"><c:out value="${data.recallDateFrom}"/> ~ <c:out value="${data.recallDateTo}"/></td>
	                                            <td class="subject"><c:out value="${data.divisionName}"/></td>
	                                            <td class="subject"><c:out value="${data.faultyInformation}"/></td>
                                            	<td class="counter"><c:out value="${data.recallCarCount}"/></td>
				                                <td class="counter"><c:out value="${data.correctCarCount}"/></td>
				                                <c:set var="recallCarPer" value="${data.correctCarCount * 100 / data.recallCarCount }" />
				                                <td class="counter"><c:out value="${recallCarPer}"/>%</td>
				                                <td class="subject uk-text-left"><a href="#" onclick="$rcInfo.event.detailView('<c:out value="${data.recallId}"/>', '1001'); return false;"><c:out value="${data.recallTitle}"/></a></td>
                                            	<td class="num"><c:out value="${data.recallId}"/></td>
                                            	<td class="subject"><c:out value="${data.recoverMethod}"/></td>
                                ...
                                <div class="btn-write">
                                    <div class="uk-text-left" style="margin-top:36px;">
                                        <button type="button" class="uk-button uk-button-default" id="isOpenY" onclick="$rcInfo.event.isOpenClick('Y')">공개</button>
                                        <button type="button" class="uk-button uk-button-default" id="isOpenN" onclick="$rcInfo.event.isOpenClick('N')">비공개</button>
                                        <button type="button" class="uk-button uk-button-default" id="snsOpenY" onclick="$rcInfo.event.snsOpenClick('Y')">SNS허용</button>
                                        <button type="button" class="uk-button uk-button-default" id="snsOpenN" onclick="$rcInfo.event.snsOpenClick('N')">SNS제외</button>
                                    </div>
                                ...
                                <ul class="uk-pagination uk-flex-center pagination" uk-margin>
                                	<c:if test="${not empty paginationInfo}"> 
                                		<ui:pagination paginationInfo="${paginationInfo}" type="custom" jsFunction="$rcInfo.event.fn_search"/>
                                    </c:if> 
                                </ul>
                            </div>
                        </div>
