...
<script type="text/javascript" src="${contextPath}/js/si/conditionRecall.js"></script>
      ...
      <input type="hidden" id="divisionCodeBt" >
		<div class="content-padder">
                        ...
                        <div>
                            <div class="uk-card uk-card-default uk-card-body">
                                <form id="srchFrm" name="searchFrm" method="POST" onsubmit="return false;" action="" class="search-condition">
									...
									<div class="uk-form-controls">
										<label class="uk-form-label">기관선택</label>
										<select id="organ" name="organ" class="uk-select w-100">
											<option value="0" <c:if test="${'0' == search.organ}">selected="selected"</c:if>>전체</option>
											<option value="1" <c:if test="${'1' == search.organ}">selected="selected"</c:if>>국토부</option>
											<option value="2" <c:if test="${'2' == search.organ}">selected="selected"</c:if>>환경부</option>
										</select>
									</div>
			...
			<c:choose><c:when test="${'Y' != search.rcType}">
				<span uk-icon="icon: microphone;"></span>리콜현황은 국토교통부리콜(안전결함관련)과 환경부리콜(배출가스관련)을 합산한 자료입니다.
			</c:when><c:otherwise>
				<span uk-icon="icon: microphone;"></span>리콜현황(전체)은 국토교통부리콜(안전결함관련)과 환경부리콜(배출가스관련)을 합산한 자료입니다.
			</c:otherwise></c:choose>
	        </div>
 
<!-- START 리콜현황 -->
<c:if test="${search.statType eq 'RC'}">
                               
			<c:choose><c:when test="${'Y' != search.rcType}">
			<h3>${search.recallYear} 월별 리콜현황</h3>
			</c:when><c:otherwise>
			<h3>연도별 리콜현황</h3>
			</c:otherwise></c:choose>
			<table class="uk-table uk-table-divider table-stat">
                <caption>${flgVal}별 리콜현황</caption>
                <colgroup>
                    <col style="width: 15%;">
                    <col style="width: 12%;">
                    <col style="width: 14%;">
                ...
                <tbody id="openTrOne">
                <c:set var="kor_tot_car_kind_cnt" value="0" />
                <c:set var="kor_tot_car_cnt" value="0" />
                <c:set var="for_tot_car_kind_cnt" value="0" />
                <c:set var="for_tot_car_cnt" value="0" />
                <c:set var="all_tot_car_kind_cnt" value="0" />
				...
				<c:forEach items="${result}" var="data" varStatus="status">
					...
					<c:choose><c:when test="${'Y' != search.rcType}">
                        ...
                        <th>${data.recallDateFrom}년<c:if test="${search.thisYear eq data.recallDateFrom}">*</c:if></th>
                    </c:otherwise></c:choose>    
                        <td><fmt:formatNumber value='${data.korProductCnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${data.korDaesu}' type='number'/></td>
                        <td><fmt:formatNumber value='${data.forProductCnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${data.forDaesu}' type='number'/></td>
                        <td><fmt:formatNumber value='${data.totProductCnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${data.totDaesu}' type='number'/></td>
                    </tr>
                	<c:set var="kor_tot_car_kind_cnt" value="${kor_tot_car_kind_cnt + data.korProductCnt}" />
                	<c:set var="kor_tot_car_cnt" value="${kor_tot_car_cnt + data.korDaesu}" />
                	<c:set var="for_tot_car_kind_cnt" value="${for_tot_car_kind_cnt + data.forProductCnt}" />
                	<c:set var="for_tot_car_cnt" value="${for_tot_car_cnt + data.forDaesu}" />
                	<c:set var="all_tot_car_kind_cnt" value="${all_tot_car_kind_cnt + data.totProductCnt}" />
            	...
            	<c:if test="${not empty result}">
                    <tr id="openTrTotOne">
                        ...
                        <td><fmt:formatNumber value='${kor_tot_car_kind_cnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${kor_tot_car_cnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${for_tot_car_kind_cnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${for_tot_car_cnt}' type='number'/></td>
                        <td><fmt:formatNumber value='${all_tot_car_kind_cnt}' type='number'/></td>
		...
		<c:choose>
			<c:when test="${search.thisYear eq search.recallYear}">
				<p>* ${search.thisYear}년 리콜현황 집계중</p>
			</c:when>
		...
		<script>
			...
			<c:forEach items="${result}" var="data"> 
				...
				data1.push("${data.korProductCnt}");
				data2.push("${data.korDaesu}");		
				data3.push("${data.forProductCnt}");
				data4.push("${data.forDaesu}");		
				data5.push("${data.totProductCnt}");
...
<c:if test="${search.statType eq 'MK'}">
			...
			<c:choose><c:when test="${'Y' != search.rcType}">
			<h3>${search.recallYear} 월별 리콜현황(제작사별)</h3>
			</c:when><c:otherwise>
			<h3>연도별 리콜현황(제작사별)</h3>
			</c:otherwise></c:choose>
			<table class="uk-table uk-table-divider table-stat">
...
<c:if test="${search.statType eq 'PD'}">
			...
			<c:choose><c:when test="${'Y' != search.rcType}">
			<h3>${search.recallYear} 월별 리콜현황(장치별)</h3>
			</c:when><c:otherwise>
			<h3>연도별 리콜현황(장치별)</h3>
			</c:otherwise></c:choose>
			<table class="uk-table uk-table-divider table-stat">
...
<c:if test="${search.statType eq 'CG'}">
			...
			<c:choose><c:when test="${'Y' != search.rcType}">
			<h3>${search.recallYear} 월별 리콜현황(차량별)</h3>
			</c:when><c:otherwise>
			<h3>연도별 리콜현황(차량별)</h3>
			</c:otherwise></c:choose>
			<table class="uk-table uk-table-divider table-stat">
		...
		<c:if test='${not empty result && search.makerCode ne "" && search.makerCode != null}'>
            <div class="uk-margin-medium-top uk-margin-medium-bottom uk-grid">
				<div class="btn-write">
